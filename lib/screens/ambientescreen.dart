...
class _AmbienteSelectionScreenState extends State<AmbienteSelectionScreen>
  ...
  List<Ambiente> allAmbiente, selectedAmbiente;
  ...
  void initState() {
    super.initState();
    allAmbiente = service.allAmbiente;
  ...
  _getUnselectedAmbiente() {
    return allAmbiente.map((ambiente) {
      return RectGetter(
        key: ambiente.key,
        child: Padding(
    ...
    if (selectedAmbiente.length > 0) {
      ...
      ambienteEndRect =
          RectGetter.getRectFromKey(firstSelectedAmbiente.key);
    } else {
  ...
  void _timerCleanup() {
    var toCleanupCount =
        allAmbiente.where((ing) => ing.name.isEmpty).toList().length;
    if (toCleanupCount > 0) {
      setState(() {
        allAmbiente.removeWhere((ing) => ing.name.isEmpty);
